...
function draw()
{
    ...
    if ((lifeOn && POSTER_WORLD.clocks.clk > 20) || (!lifeOn && POSTER_WORLD.clocks.clk > 60))
    {
        POSTER_WORLD.clocks.clk = 0;
        POSTER_WORLD.bools.life = !POSTER_WORLD.bools.life;
    }
    POSTER_WORLD.clocks.clk++;
    if (isBackgroundOn)
        background(0);
    else
    {
    ...
    if (lifeOn)
    {
        ...
        {
            POSTER_WORLD.letters[i].expandNodes(volAgitation);
            if (centroid < 600)
            {
            ...
            POSTER_WORLD.globals.volLev.show(volAgitation, width, height, POSTER_WORLD);
        }
    }
    else
        centroid = FMID;
    // var bri = map(constrain(centroid, FMIN, FMAX), FMIN, FMAX, MINHSB, MAXHSB);
    // d_showLetters(0, 255, 100 + 100 * vol, bri, 1);
    d_showLetters(0, 255, 255, 255);
    ...
    var mean_freq_indexMID = FMID / (nyquist / spectrum.length);

...
view.onFrame = function(event)
{
    ...
    //     var tmp = timestamps.reduce(function(a, b){return (a + b);});;
    //     tmp *= 1000;
...
function updateVisual()
{
    ...
    if (posLen != wg.pos.length)
    {
        if (wg.pos.length < posLen)
        {
            ...
            {
                var rad = wg.radii[i] * wg.scaleFactor;
                circles[i].bounds.width = radius * rad * 2;
                circles[i].bounds.height = radius * rad * 2;
            }
        }
        else
        {

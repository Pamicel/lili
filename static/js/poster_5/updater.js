...
wg.mapParticuleGroup = function(iterateArray, func, groupN)
{
...
// {
//     if (letterN)
//         return (wg.mapParticuleGroup(wg.letters, wg.centerOfMass, letterN));
// };

// wg.centerOfMassWordN = function(wordN)
// {
//     if (wordN)
//         return (wg.mapParticuleGroup(wg.words, wg.centerOfMass, wordN));
// };
...
{
    wg.mapParticuleGroup(iterateArray, () => {for (let i in wg.offset) {wg.offset[i] += 10;} }, groupN);
    // wg.mapParticuleGroup(iterateArray, () => {for (i in wg.forces) {wg.forces[i] = 1;} }, groupN);
    // wg.mapParticuleGroup(iterateArray, wg.applyForces, groupN)
};

wg.extend = function(iterateArray, extentionArray, groupN)
{
    wg.mapParticuleGroup(iterateArray, (firstPart, lastPart) => {
        var x, y;
...
wg.extendY = function(iterateArray, groupN)
{
    wg.mapParticuleGroup(iterateArray, (firstPart, lastPart) => {
        var y;
        var center = wg.centerOfMass(firstPart, lastPart);
        ...
        {
            y = firstPart * 2 + 1;
            wg.offset[y] += (wg.pos[y] - center[1]) / 5;
            firstPart++;
        }
    }, groupN);
};
